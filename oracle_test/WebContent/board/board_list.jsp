...
try{
	...
	//1.게시판의 레코드 갯수 계산
	String sql_count = 
		"select count(*) from board where "+searchkey +" like ? ";
...
<input type="button" value="조회"   onclick="javascript:board_search();"/>
...
//			tot_page / block_size : tot_page/block_size+1;
	...
	if( block_end > tot_page ) block_end = tot_page;
...
<!-- 반복문 -->	
<%
/* String sql = "select idx, name, subject, hit, post_date, cnt "
	+" from " 
	+ "( select rownum as rn, idx, name, subject, hit "
...
String sql = 
	"select * from board_view where rn between ? and ? ";
...
%>
	<tr>
		...
		<td><%=name%></td>
		<td>
<a href="board_view.jsp?idx=<%=idx%>&cur_page=<%=cur_page%>">
				<%=subject%></a>
				<% if ( cnt > 0) { %>	
...
%>		
	<a href="board_list.jsp?cur_page=<%=i%>&searchkey=<%=searchkey%>&search=<%=search%>"><%=i%></a>&nbsp;
<%
	}
}
...
%>
<!-- [다음] 블록 링크  -->
	<a href="board_list.jsp?cur_page=<%=next_page%>">[다음]</a>
<%
}
%>	
</div>
