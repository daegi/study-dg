...
public class BoardDAO {
	...
	public int insertBoard(BoardDTO dto, String mode) {
		...
		try {
			...
			sql="INSERT INTO board (" + fields + ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public int dataCount(String searchKey, String searchValue) {
        ...
        try {
        	if(searchValue.length()==0)
        		...
        		sql="SELECT NVL(COUNT(*), 0) FROM board b JOIN join_basic m ON b.userId=m.userId WHERE " + searchKey + " LIKE '%' || ? || '%' ";
    ...
    public BoardDTO preReadBoard(String searchKey, String searchValue, int groupNum, int orderNo) {
        ...
        try {
            if(searchValue.length() != 0) {
                sb.append("SELECT ROWNUM, data.* FROM (");
                sb.append("     SELECT b.userId, userName, num, subject ");
        		sb.append("              FROM board b JOIN join_basic m ON b.userId=m.userId");
                sb.append("        WHERE ( " + searchKey + "  LIKE '%'|| ? ||'%') AND ");
                sb.append("        (( groupNum = ? AND orderNo < ?) ");
                sb.append("         OR (groupNum > ? )) ");
                sb.append("         ORDER BY groupNum ASC, orderNo DESC) data WHERE ROWNUM = 1 ");
    ...
    public BoardDTO nextReadBoard(String searchKey, String searchValue, int groupNum, int orderNo) {
        ...
        try {
            if(searchValue.length() != 0) {
                ...
                sb.append("     (( groupNum = ? AND orderNo > ?) ");
                sb.append("         OR (groupNum < ? )) ");
                sb.append("         ORDER BY groupNum DESC, orderNo ASC) data WHERE ROWNUM = 1 ");
	...
	public int deleteBoard(int num) {
		...
		sql="DELETE FROM board WHERE num IN (SELECT num FROM board START WITH  num = ? CONNECT BY PRIOR num = parent)";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			result = pstmt.executeUpdate();
