...
public class BoardServlet extends HttpServlet {
	...
	protected void process(HttpServletRequest req, HttpServletResponse resp)
		...
		if(uri.indexOf("list.do")!=-1) {
			...
			int total_page = myUtil.getPageCount(numPerPage, 
					dataCount);
	        ...
	        String articleUrl = cp+"/board/article.do?pageNum=" + current_page;
	        if(params.length()!=0) {
	        	listUrl+="?"+params;
			...
			try {
				...
				if(mreq.getFilesystemName("upload")!=null && mreq.getFilesystemName("upload").length()!=0) {
			...
			try {
				...
				int groupNum=Integer.parseInt(
						mreq.getParameter("groupNum"));
				int depth=Integer.parseInt(
						mreq.getParameter("depth"));
				int orderNo=Integer.parseInt(
						mreq.getParameter("orderNo"));
				int parent=Integer.parseInt(
						mreq.getParameter("parent"));
			...
			try {
				...
				if(mreq.getFilesystemName("upload")!=null && mreq.getFilesystemName("upload").length()!=0) {
					if(vo.getSaveFileName()!=null) {
						...
						fm.doFileDelete(vo.getSaveFileName(), pathname);
					}
